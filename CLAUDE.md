...
`whitening.json` at the repo root is read by the whitening packer, not by the app: it sets
the closed-network repo name and the exclude globs that the packer writes into the zip's
`config.json` (which the Whitening module reads in place of the zip filename).
